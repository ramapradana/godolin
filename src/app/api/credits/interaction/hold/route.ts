...
export async function POST(request: NextRequest) {
  try {
    ...
    const { amount, reference_id, expires_in_minutes = 30 } = body;
    ...
    try {
      ...
      console.log('Interaction credit hold created', {
        userId,
        holdId,
        amount,
        reference_id,
      ...
      console.error('Unexpected error in interaction credit hold:', error);
      return NextResponse.json({ 
    ...
    console.error('Error in interaction credit hold API:', error);
